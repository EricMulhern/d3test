  ...
  const ref = useD3(
    (svg) => {
      ...
      let margin = {top: 20, right: 20, bottom: 30, left: 50},
          ...
          width = 960 - margin.left - margin.right, // figure out how to access width&height of parent
          height = 500 - margin.top - margin.bottom;
  
      const keys = [];
      for (let key in data[0]) { // make more abstractable
        if (key !== 'month') keys.push(key);
      }

      let x = d3.scaleTime().domain([data[0].month, data[data.length-1].month]).range([0, width]), 
          y = d3.scaleLinear().domain([0, 60]).range([height, 0]), // CUSTOMIZE Y DOMAIN BASED ON THE HIGHEST Y1 VALUE? IS THERE A BETTER WAY?
          z = d3.scaleOrdinal(d3.schemeCategory10); // COLORS
      ...
      x.domain(d3.extent(data, function(d) { return +Date.parse(d.month); }));
      z.domain(keys);
      stack.keys(keys);

      let layer = g.selectAll(".layer")
        .data(stack(data)) // what if we don't stack?
        // .data([data]) // still need to transform the data
        .enter().append("g")
          .attr("class", "layer");
